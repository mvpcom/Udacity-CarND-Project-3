...
def processFunc(image):
    #image = cv2.resize(image,(image.shape[0],images.shape[1]))
    #image = image/255. - 0.5
    #return self.roi(cv2.cvtColor(img, cv2.COLOR_RGB2YUV))
    #return self.roi(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    #image = self.roi(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    #image = image / 255. - .5
    #return image
    #image = img
